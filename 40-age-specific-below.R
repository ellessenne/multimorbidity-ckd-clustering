...
ggsave(plot = ep, filename = "figures/40-age-specific-below-elbow-plots.pdf", height = 5, width = 5 * 29.7 / 21, device = cairo_pdf)
ggsave(plot = ep, filename = "figures/40-age-specific-below-elbow-plots.png", height = 5, width = 5 * 29.7 / 21, device = ragg::agg_png, res = 600, units = "in")
...
ggsave(plot = dep, filename = "figures/40-age-specific-below-elbow-plots-grad.pdf", height = 5, width = 5 * 29.7 / 21, device = cairo_pdf)
ggsave(plot = dep, filename = "figures/40-age-specific-below-elbow-plots-grad.png", height = 5, width = 5 * 29.7 / 21, device = ragg::agg_png, res = 600, units = "in")

# Export all clustering results too
saveRDS(object = full_res, file = "data/40-age-specific-below-fitted-clusters.RDS")

# Summarise the optimal clustering
# We pick 'n_to_pick' clusters for each eGFR value
n_to_pick <- c("5", "7", "7", "5", "5", "5")
...
summ_mm %>%
  ...
  arrange(eGFR, Condition) %>%
  write_csv(file = "data/40-age-specific-below-proportion-conditions-by-clusters.csv")
# Heatmap
p_heat <- summ_mm %>%
...
ggsave(plot = p_heat, filename = "figures/40-age-specific-below-heatmap-by-clusters.pdf", width = 6, height = 6 * sqrt(2), device = cairo_pdf)
ggsave(plot = p_heat, filename = "figures/40-age-specific-below-heatmap-by-clusters.png", width = 6, height = 6 * sqrt(2), device = ragg::agg_png, res = 600, units = "in")
...
p_def <- ggplot(oedf, aes(x = name, y = Condition, fill = factor(value))) +
  ...
  theme(legend.position = "top") +
  labs(x = "Cluster", fill = "Proportion with Condition", caption = "Analysis for age < 65")
ggsave(plot = p_def, filename = "figures/40-age-specific-below-defining-conditions-by-clusters.pdf", width = 6, height = 6 * sqrt(2), device = cairo_pdf)
ggsave(plot = p_def, filename = "figures/40-age-specific-below-defining-conditions-by-clusters.png", width = 6, height = 6 * sqrt(2), device = ragg::agg_png, res = 600, units = "in")
...
names(.palette) <- NULL
cdesc <- readxl::read_excel("data/40-age-specific-below-SCREAM-cluster-descriptions-v2.xlsx")
...
ggsave(plot = p_prop, filename = "figures/40-age-specific-below-cluster-wise-proportion.pdf", height = 5, width = 5 * sqrt(2), device = cairo_pdf)
ggsave(plot = p_prop, filename = "figures/40-age-specific-below-cluster-wise-proportion.png", height = 5, width = 5 * sqrt(2), device = ragg::agg_png, res = 600, units = "in")
